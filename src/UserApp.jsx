...
const mapRealDataToLessons = (realData) => {
  ...
  realData.forEach((item) => {
    ...
    const staticLesson = LESSONS[lang]?.[levelNumber - 1] || {}

    mapped[lang].push({
      id: `${lang.toLowerCase()}${levelNumber}`,
      level: levelNumber,
      ...
      {lines.map((line, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            borderLeft: isHeader ? `3.5px solid ${cfg.primary}` : 'none',
            padding: isHeader ? '8px 12px' : '4px 12px 4px 24px',
            borderRadius: isHeader ? 8 : 0,
            fontSize: isHeader ? 15 : 14,
            fontWeight: isHeader ? 700 : 500,
            color: isHeader ? '#1e293b' : '#475569',
            lineHeight: 1.5,
            display: 'flex',
            alignItems: 'center',
            gap: 8,
...
// ─── Home View ────────────────────────────────────────────────────────────────
function HomeView({ user, xp, streak, completed, setActive, setLearnLang, lessonsData }) {
  const total = Object.values(completed).flat().length
  const langStats = Object.entries(lessonsData).map(([lang, ls]) => ({
...
// ─── Explore View ─────────────────────────────────────────────────────────────
function ExploreView({ completed, setActive, setLearnLang, setLearnLesson, lessonsData }) {
...
function LearnView({ learnLang, setLearnLang, learnLesson, setLearnLesson, completed, onComplete, lessonsData }) {
  const [step, setStep] = useState(0) // 0=vocab, 1=grammar, 2=practice
  const [showAns, setShowAns] = useState(false)
  const [xpAnim, setXpAnim] = useState(false)

  const lessons = lessonsData[learnLang] || []
      ...
      <div className="fade-up" key={step}>
        {step === 0 && (
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: 24 }}>
            ...
            {renderFormattedVocab(lesson.vocab, cfg)}
...
// ─── Progress View ────────────────────────────────────────────────────────────
function ProgressView({ xp, streak, completed, lessonsData }) {
    ...
    <div className="fade-up" style={{ padding: '28px 28px 40px' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '20px 24px', marginBottom: 20 }}>
        ...
        {Object.entries(LANG).map(([k, c]) => {
          ...
          const tot = lessonsData[k].length
          const pct = Math.round((done / tot) * 100)
          return (
...
// ─── Profile View ─────────────────────────────────────────────────────────────
function ProfileView({ user, xp, streak, completed, onLogout, lessonsData }) {
  const total = Object.values(completed).flat().length
  const level = getLevel(xp)
...
export default function UserApp({ user, onLogout }) {
  ...
  const renderView = () => {
    switch (active) {
      case 'home': return <HomeView user={user} xp={xp} streak={streak} completed={completed} setActive={setActive} setLearnLang={setLearnLang} lessonsData={lessonsData} />
      case 'explore': return <ExploreView completed={completed} setActive={setActive} setLearnLang={setLearnLang} setLearnLesson={setLearnLesson} lessonsData={lessonsData} />
      case 'learn': return <LearnView learnLang={learnLang} setLearnLang={setLearnLang} learnLesson={learnLesson} setLearnLesson={setLearnLesson} completed={completed} onComplete={handleComplete} lessonsData={lessonsData} />
      case 'live': return <LiveView />
      case 'progress': return <ProgressView xp={xp} streak={streak} completed={completed} lessonsData={lessonsData} />
      case 'profile': return <ProfileView user={user} xp={xp} streak={streak} completed={completed} onLogout={onLogout} lessonsData={lessonsData} />
      default: return <HomeView user={user} xp={xp} streak={streak} completed={completed} setActive={setActive} setLearnLang={setLearnLang} lessonsData={lessonsData} />
    }
  }
