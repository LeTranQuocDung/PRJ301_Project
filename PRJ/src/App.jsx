...
import LiveRoomsView from "./LiveRoomsView";
...
// ─── Primitives ──────────────────────────────────────────────────
export const Badge = ({ children, color = C.primary, bg = C.primaryLight, style }) => (
...
export const Btn = ({ children, onClick, v = "primary", sm, disabled, fullWidth, style = {} }) => {
  const map = {
...
export const CardHead = ({ icon, title, action, accent }) => (
...
function Sidebar({ active, setActive, currentUser, onLogout }) {
  ...
  return (
    <aside style={{ width: 210, minWidth: 210, background: C.sidebar, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", height: "100vh", overflowY: "auto", flexShrink: 0 }}>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 8, background: "#fafafa" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: "50%", background: currentUser.role === 'Teacher' ? C.purpleLight : currentUser.role === 'Influencer' ? C.yellowLight : C.primaryLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, border: `1px solid ${currentUser.role === 'Teacher' ? C.purple : currentUser.role === 'Influencer' ? C.yellow : C.primary}` }}>
            {currentUser.avatar || (currentUser.role === 'Teacher' ? "👨‍🏫" : currentUser.role === 'Influencer' ? "🌟" : "👤")}
          </div>
          <div style={{ flex: 1, overflow: "hidden" }}>
            <div style={{ fontWeight: 600, fontSize: 13, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentUser.name}</div>
            <div style={{ fontSize: 10, color: currentUser.role === 'Teacher' ? C.purple : currentUser.role === 'Influencer' ? "#b45309" : C.primary, fontWeight: 600 }}>{currentUser.role}</div>
          </div>
        </div>
        <Btn sm v="danger" onClick={onLogout} fullWidth style={{ marginTop: 4 }}>Đăng xuất</Btn>
      </div>
    </aside>
  );
}
...
// --- Agora Config -------------------------------------------------------
export const AGORA_APP_ID  = "ca82570aa4a3464aadca4e28ee1d73b9";
export const AGORA_CHANNEL = "lucy_room_1";
// Temp token (valid ~24h). Replace via Agora Console > Generate Temp Token
export const AGORA_TEMP_TOKEN = "006ca82570aa4a3464aadca4e28ee1d73b9IACc5s3b/IwXIquJv0NUyYgLxo3PXKy0esWGIFIZ5GaFrJrnejAAAAAAIgCzgFZ7Vp06agQAAQBWnTpqAgBWnTpqAwBWnTpqBABWnTpq";
...
export default function App() {
    ...
    switch (active) {
      ...
      case "live-rooms": 
        return selectedRoomId ? (
          <InRoomView roomId={selectedRoomId} currentUser={currentUser} onLeave={() => setSelectedRoomId(null)} />
        ) : (
          <LiveRoomsView currentUser={currentUser} onJoinRoom={(id) => setSelectedRoomId(id)} />
        );
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Inter', 'Segoe UI', system-ui, sans-serif", fontSize: 14, color: C.text, overflow: "hidden" }}>
      ...
      `}</style>
      <Sidebar active={active} setActive={setActive} currentUser={currentUser} onLogout={handleLogout}/>
